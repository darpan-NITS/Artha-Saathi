...
export default function FutureShock({ sessionId }: Props) {
  ...
  const diff     = data.difference_5yr
  const positive = diff > 0

  // Merge both timelines into one chart dataset
  const chartData = data.current_path.map((p: any, i: number) => ({
    year: p.year,
    current:   p.corpus,
    optimised: data.optimised_path[i]?.corpus ?? p.corpus
  }))
  ...
  return (
    <div className="space-y-4 tab-content">
      ...
      {/* Headline number */}
      <div className={`rounded-2xl p-5 text-white bg-gradient-to-r ${
        ...
        <div className="flex gap-6 mt-4">
          <div>
            ...
            <p className="text-lg font-bold">{fmt(data.corpus_current_5yr)}</p>
          </div>
          <div className="w-px bg-white/20" />
          <div>
            <p className="text-xs text-white/60">With this plan</p>
            <p className="text-lg font-bold">{fmt(data.corpus_optimised_5yr)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="panel-card p-5">
            ...
            <AreaChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                formatter={(v: number, name: string) => [
                  fmt(v),
                  name === "optimised" ? "With plan" : "Without action"
                ]}
              ...
              <Area
                ...
                fill="url(#curGrad)" name="current"
                strokeDasharray="5 3"
              />
              <Area
                type="monotone" dataKey="optimised"
                stroke="#059669" strokeWidth={2.5}
                fill="url(#optGrad)" name="optimised"
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="flex gap-4 mt-3 justify-center">
          <div className="flex items-center gap-1.5 text-xs text-gray-500">
            <div className="w-4 h-0.5 bg-red-400 rounded" style={{borderTop: "2px dashed #f87171"}} />
            Without action
          </div>
          <div className="flex items-center gap-1.5 text-xs text-gray-500">
